...
export default function GrammarExplorer({
  ...
  const startExercise = (point: GrammarPoint) => {
    setActiveExercise(point);
  };

  return (
    <>
      {/* Masquer la liste quand un exercice est actif */}
      {!activeExercise && (
      ...
      {/* Modal d'exercice - passe la note pour afficher l'explication */}
      {activeExercise && activeExercise.data?.drills && (
        <GrammarDrill
