      ...
      <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 overflow-y-auto">
        <div className="bg-gray-800 rounded-2xl p-6 max-w-3xl w-full my-8">
          ...
          <div className="bg-gray-900 rounded-lg p-6 mb-6">
            <div className="text-base leading-relaxed whitespace-pre-wrap" style={{ color: '#e5e7eb', lineHeight: '1.8' }}>
              {note}
            </div>
          </div>
          <div className="flex gap-3">
            <button onClick={() => setShowExplanation(false)} className="flex-1 bg-blue-600 hover:bg-blue-700 px-6 py-4 rounded-lg font-semibold text-lg">
              ✏️ Commencer les exercices ({drills.length})
            </button>
            <button onClick={onClose} className="px-6 py-4 bg-gray-700 hover:bg-gray-600 rounded-lg font-semibold">
              Retour
            </button>
          </div>
        </div>
    ...
    return (
      <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
        <div className="bg-gray-800 rounded-2xl p-8 max-w-md w-full text-center">
          ...
          <button onClick={onClose} className="bg-blue-600 hover:bg-blue-700 px-6 py-3 rounded-lg font-semibold">
            Retour à la liste
          </button>
        </div>
      </div>
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-800 rounded-2xl p-6 max-w-2xl w-full">
        ...
        <div className="mb-6 text-center">
          <span className="text-lg">Score: <span className="font-bold text-green-400">{score}</span> / {drills.length}</span>
        </div>
        <div className="bg-gray-900 rounded-lg p-6 mb-6">
          <p className="text-lg" dangerouslySetInnerHTML={{ __html: currentDrill.prompt }} />
        </div>
        <input type="text" value={userAnswer} onChange={(e) => setUserAnswer(e.target.value)}
          onKeyPress={handleKeyPress} placeholder="Tape ta réponse ici..." disabled={feedback !== null} autoFocus
        ...
        />
        {feedback && (
          <div className={`mt-4 p-4 rounded-lg ${feedback === "correct" ? "bg-green-900/50 border border-green-600" : "bg-red-900/50 border border-red-600"}`}>
            {feedback === "correct" ? (
              <p className="text-green-400 font-semibold">✓ Correct!</p>
            ) : (
              <div>
                <p className="text-red-400 font-semibold">✗ Incorrect</p>
                <p className="text-sm mt-2">Bonne réponse: <span className="font-bold">{currentDrill.answer}</span></p>
              </div>
            )}
          </div>
        )}
        <div className="mt-6 flex gap-3">
          {feedback === null ? (
            <button onClick={checkAnswer} disabled={!userAnswer.trim()} className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-700 px-6 py-3 rounded-lg font-semibold">
              Vérifier
            </button>
          ) : (
            <button onClick={nextDrill} className="flex-1 bg-blue-600 hover:bg-blue-700 px-6 py-3 rounded-lg font-semibold">
